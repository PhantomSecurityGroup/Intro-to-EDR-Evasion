...
Sw3NtProtectVirtualMemory PROC
	...
	mov [rsp+32], r9
	sub rsp, 28h
	mov ecx, 049E33F0Dh        ; Load function hash into ECX.
	call SW3_GetSyscallAddress              ; Resolve function hash into syscall offset.
	mov r11, rax                           ; Save the address of the syscall
	mov ecx, 049E33F0Dh        ; Re-Load function hash into ECX (optional).
	call SW3_GetSyscallNumber              ; Resolve function hash into syscall number.
	add rsp, 28h
	mov rcx, [rsp+8]                      ; Restore registers.
	mov rdx, [rsp+16]
